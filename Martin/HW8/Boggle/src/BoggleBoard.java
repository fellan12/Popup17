public class BoggleBoard {
	...
	public Results getResults() {
		...
		for (int i = 0; i < board.length; i++) {
			dfs(1, i, "", new boolean[16], 0, results);
		}
		return results;
	}
	
	private void dfs(int currentDepth, int currentIndex, String currentString, boolean[] visited, int trieIndex, Results results) {
		char nextChar = board[currentIndex];
        if (!trie.isPrefixGen(trieIndex, nextChar))
            return;
        currentString += nextChar;
        trieIndex = trie.getNextNodeIndex(trieIndex, nextChar);
        visited[currentIndex] = true;
        if (currentDepth >= 2 && trie.isWord(currentString))
            results.add(currentString);
        if (MAX_DEPTH > currentDepth) {
            for(int i : getNeighborIndexes(currentIndex)) {
                if (!visited[i])
                    dfs(currentDepth+1, i, currentString, visited, trieIndex, results);
            }
        }
